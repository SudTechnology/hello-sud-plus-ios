...
- (HSGameItem *)getSceneGameInfo:(int64_t)gameId {
    NSArray <HSGameItem *> *dataArr = [AppService.shared getGameListByTab:0];
...
- (NSMutableDictionary<NSString *, NSArray <HSGameItem *> *> *)tabGameMap {
    if (!_tabGameMap) {
        _tabGameMap = NSMutableDictionary.new;
    }
    return _tabGameMap;
}
...
- (BOOL)isCurrentRtcSurpportVideo {
    return [self isSameRtc:self.configModel.agoraCfg rtcType: self.rtcType] || [self isSameRtc:self.configModel.zegoCfg rtcType: self.rtcType];
}
@end

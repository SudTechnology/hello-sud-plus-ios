...
+ (void)reqEnterRoom:(long)roomId isFromCreate:(BOOL)isFromCreate extData:(NSDictionary *)extData success:(nullable EmptyBlock)success fail:(nullable ErrorBlock)fail {
    ...
    ReqEnterRoomParamModel *paramModel = ReqEnterRoomParamModel.new;
    paramModel.roomId = roomId;
    paramModel.isFromCreate = isFromCreate;
    paramModel.extData = extData;

    // 如果存在挂起房间，则退出当前挂起房间
    if ([SuspendRoomView isShowSuspend]) {
        [SuspendRoomView exitRoom:^{
            [self handleEnterRoomWithParamModel:paramModel success:success fail:fail];
        }];
        return;
    }
    [self handleEnterRoomWithParamModel:paramModel success:success fail:fail];
...
/// 处理进入房间
/// @param roomId 房间ID
+ (void)handleEnterRoomWithParamModel:(ReqEnterRoomParamModel *)paramModel success:(nullable EmptyBlock)success fail:(nullable ErrorBlock)fail {
    WeakSelf
    ...
    NSMutableDictionary *dicParam = NSMutableDictionary.new;
    dicParam[@"roomId"] = @(paramModel.roomId);
    if (AppService.shared.rtcType.length > 0) {
        dicParam[@"rtcType"] = AppService.shared.rtcType;
    }
    [HSHttpService postRequestWithURL:kINTERACTURL(@"room/enter-room/v1") param:dicParam respClass:EnterRoomModel.class showErrorToast:YES success:^(BaseRespModel *resp) {
        isReqEnter = NO;
        EnterRoomModel *model = (EnterRoomModel *) resp;
        model.isFromCreate = paramModel.isFromCreate;
        ...
        if (paramModel.extData) {
            [model.dicExtData setDictionary:paramModel.extData];
        }
        SceneParamModel *sceneParamModel = SceneParamModel.new;
        sceneParamModel.sceneType = model.sceneType;
        sceneParamModel.configModel = config;
        sceneParamModel.loadType = paramModel.loadType;
        sceneParamModel.tabType = paramModel.tabType;
        config.loadGameType = paramModel.loadType;
        config.tabType = paramModel.tabType;
        // 兼容房间列表进入时无法获取加载类型情况，需要从游戏列表中获取
        if (model.gameId > 0) {
...
+ (void)reqMatchRoomWithParamModel:(ReqMatchRoomParamModel *)paramModel {
    ...
    isMatchingRoom = YES;
    NSMutableDictionary *dicParam = NSMutableDictionary.new;
    dicParam[@"gameId"] = @(paramModel.gameId);
    dicParam[@"sceneType"] = @(paramModel.sceneType);
    if (AppService.shared.rtcType.length > 0) {
        dicParam[@"rtcType"] = AppService.shared.rtcType;
    }
    if (paramModel.sceneType == SceneTypeTicket) {
        dicParam[@"gameLevel"] = @(paramModel.gameLevel);
    }
    [HSHttpService postRequestWithURL:kINTERACTURL(@"room/match-room/v1") param:dicParam respClass:MatchRoomModel.class showErrorToast:YES success:^(BaseRespModel *resp) {
        MatchRoomModel *model = (MatchRoomModel *) resp;
        
        ReqEnterRoomParamModel *enterRoomParamModel = ReqEnterRoomParamModel.new;
        enterRoomParamModel.roomId = model.roomId;
        enterRoomParamModel.isFromCreate = NO;
        enterRoomParamModel.loadType = paramModel.loadType;
        enterRoomParamModel.tabType = paramModel.tabType;
        [AudioRoomService reqEnterRoomWithParamModel:enterRoomParamModel success:^{
            isMatchingRoom = NO;
        } fail:^(NSError *error) {
            [ToastUtil show:[error debugDescription]];
            isMatchingRoom = NO;

...
- (void)requestData {
    ...
    [HSHttpService postRequestWithURL:kINTERACTURL(@"game/list/v1") param:@{} respClass:GameListModel.class showErrorToast:false success:^(BaseRespModel *resp) {
        ...
        [AppService.shared addGameListToTab:0 gameList:originalGameArr];
        [AppService.shared addSceneCacheList:model.sceneList toTabId:0];
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (self.headerSceneList[indexPath.section].sceneId == SceneTypeTicket) {
        ...
        if ((m.sceneId == SceneTypeDanmaku || m.sceneId == SceneTypeVertical) && ![AppService.shared isCurrentRtcSurpportVideo]) {

...
@interface OneOneAudioMicroView ()
...
@implementation OneOneAudioMicroView

- (void)setMicType:(OneOneMicType)micType {
    _micType = micType;
...
- (void)dtLayoutViews {
    ...
    [self.headerView dt_cornerRadius:40];
...
//    }];
    ...
    [self.gameStateLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.centerY.equalTo(self.headerView.mas_bottom).offset(-3);
    }];
...
- (void)dtConfigEvents {
    ...
    [[NSNotificationCenter defaultCenter] addObserverForName:NTF_MIC_CHANGED object:nil queue:NSOperationQueue.mainQueue usingBlock:^(NSNotification *_Nonnull note) {
        ...
        if ([msgModel isKindOfClass:RoomCmdUpMicModel.class]) {
            ...
            [weakSelf dtUpdateUI];
            if (weakSelf.micUserChangedBlock) {
                weakSelf.micUserChangedBlock(weakSelf.model);
            }
        } else {
            [weakSelf dtUpdateUI];
        }
    ...
    [[NSNotificationCenter defaultCenter] addObserverForName:NTF_SEND_GIFT_USER_CHANGED object:nil queue:NSOperationQueue.mainQueue usingBlock:^(NSNotification *_Nonnull note) {
        ...
        if ([micModel isKindOfClass:AudioRoomMicModel.class]) {
            ...
            if (micModel.micIndex == weakSelf.model.micIndex) {
                weakSelf.giftImageView.hidden = micModel.isSelected && weakSelf.micType == OneOneAudioMic ? NO : YES;
            }
        } else {
            [weakSelf dtUpdateUI];
        }
...
- (void)dtConfigUI {
    [super dtConfigUI];
    /// 默认游戏
    self.micType = OneOneGameMic;
}

- (void)dtUpdateUI {
    [self.gameimgLabel setHidden:true];
    if (self.model.user == nil) {
        self.headerView.image = [UIImage imageNamed:@"oneone_add"];
        if (self.micType == OneOneAudioMic) {
    ...
    self.giftImageView.hidden = self.model.isSelected && self.micType == OneOneAudioMic && kAudioRoomService.sceneType != SceneTypeOrder ? NO : YES;
    BOOL isMe = [AppService.shared.login.loginUserInfo isMeByUserID:self.model.user.userID];
    NSString *showUserName = isMe ? [NSString stringWithFormat:@"%@\n(我)", self.model.user.name] : self.model.user.name;
    [self showUserName:showUserName showOwner:self.model.user.roleType == 1 && self.micType == OneOneAudioMic];
...
- (void)updateOrderUI {
    ...
    if (kAudioRoomService.sceneType == SceneTypeOrder && self.micType == OneOneAudioMic) {
        if ([kAudioRoomService.currentRoomVC.sudFSMMGDecorator isPlayerInGame:self.model.user.userID]) {
            /// 点单场景下的 游戏中
            DDLogDebug(@"is game playing:%@, user id:%@", @(kAudioRoomService.currentRoomVC.sudFSMMGDecorator.isPlaying), self.model.user.userID);
...
- (void)updateGameUI {
    ...
    if (self.micType == OneOneGameMic) {
        ...
        /// 设置队长状态 - （队长有且只有一个）
        [self.gameCaptainView setHidden:true];
        BOOL isCaptain = [kAudioRoomService.currentRoomVC.sudFSMMGDecorator isPlayerIsCaptain:self.model.user.userID];
        if (isCaptain) {
        ...
        if (![kAudioRoomService.currentRoomVC.sudFSMMGDecorator isPlayerInGame:self.model.user.userID]) {
            ...
            // 在游戏大厅中
            if ([kAudioRoomService.currentRoomVC.sudFSMMGDecorator isPlayerIsPlaying:self.model.user.userID]) {
                // 正在玩游戏
                [self hiddenGameStateViews:false];
                [self.gamingImageView setHidden:false];
            } else if ([kAudioRoomService.currentRoomVC.sudFSMMGDecorator isPlayerIsReady:self.model.user.userID]) {
                // 准备
...
//    attrName.yy_font = [UIFont systemFontOfSize:14 weight:UIFontWeightMedium];
...
- (SDAnimatedImageView *)headerView {
    if (!_headerView) {
        ...
        _headerView.image = [UIImage imageNamed:@"oneone_add"];
...
- (UILabel *)gameStateLabel {
    if (!_gameStateLabel) {
        ...
        _gameStateLabel.font = UIFONT_REGULAR(10);

...
@property (nonatomic, strong) MarqueeLabel *desTitleLabel;
...
- (void)joinEvent:(UIButton *)btn {
    if (self.onJoinCallBack) {
        if (self.remindImgView.isSelected) {
            [kTicketService savePopTicketJoin:true];
        }
...
- (void)dtLayoutViews {
    ...
    [self.desTitleLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.trailing.equalTo(self.goldBgView.mas_leading).offset(-2);
...
- (UILabel *)titleLabel {
    if (!_titleLabel) {
        ...
        _titleLabel.textColor = [UIColor dt_colorWithHexString:@"#FFCE76" alpha:1];
        _titleLabel.font = UIFONT_BOLD(14);
    }
    return _titleLabel;
}

- (MarqueeLabel *)desTitleLabel {
    if (!_desTitleLabel) {
        _desTitleLabel = [[MarqueeLabel alloc] init];
        _desTitleLabel.textColor = [UIColor dt_colorWithHexString:@"#FFCE76" alpha:1];
        _desTitleLabel.font = UIFONT_BOLD(14);
        _desTitleLabel.trailingBuffer = 10;
    }

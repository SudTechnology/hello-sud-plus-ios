...
#import "ForeignWalletSelectCell.h"

@interface ForeignWalletSelectCell ()
...
@implementation ForeignWalletSelectCell
...
- (void)dtLayoutViews {
    ...
    [self.subTitleLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.height.mas_greaterThanOrEqualTo(CGSizeZero);
        make.trailing.equalTo(self.bindBtn.mas_leading).offset(-23);
    }];
...
- (void)dtUpdateUI {
    ...
    BOOL isBind = [HsNFTPreferences.shared isBindWalletWithType: m.type];
    NSString *phone = [HsNFTPreferences.shared getBindWalletAddressByWalletType: m.type];
    if (isBind) {

...
- (void)onGameStateChange:(nonnull id <ISudFSMStateHandle>)handle state:(nonnull NSString *)state dataJson:(nonnull NSString *)dataJson {
    ...
    } else if ([state isEqualToString:MG_COMMON_GAME_LOAD_CONFIG_UI_RESULT]) {
        MgCommonGameLoadConfigUiResult *m = [MgCommonGameLoadConfigUiResult mj_objectWithKeyValues:dataJson];
        if (self.listener != nil && [self.listener respondsToSelector:@selector(onGameMgCommonGameLoadConfigUiResult:model:)]) {
            [self.listener onGameMgCommonGameLoadConfigUiResult:handle model:m];
            return;
        }
    }

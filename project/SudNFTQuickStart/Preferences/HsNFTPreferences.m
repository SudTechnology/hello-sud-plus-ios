...
- (void)prepare {
    ...
    _userId = [NSUserDefaults.standardUserDefaults stringForKey:@"quickstart_userId"];
    if (!_userId) {
        _userId = [NSString stringWithFormat:@"%@", @(arc4random())];
        [NSUserDefaults.standardUserDefaults setValue:_userId forKey:@"quickstart_userId"];
...
- (NSString *)getBindUserTokenByWalletType:(NSInteger)walletType {
    NSString *token = [NSUserDefaults.standardUserDefaults objectForKey:[NSString stringWithFormat:@"%@_%@", kKeyBindUserToken, @(walletType)]];
    return token;
}

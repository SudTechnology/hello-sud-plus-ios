...
#import "SQSSelectEtherChainsView.h"
...
@property(nonatomic, strong) NSArray<SQSNFTListCellModel *> *nftCellModelList;
...
- (void)updateShowInfo {
    ...
    for (int i = 0; i < self.iconImageViewList.count; ++i) {
        ...
        if (self.nftCellModelList.count > i) {
            SQSNFTListCellModel *cellModel = self.nftCellModelList[i];
            [self showLoadAnimate:iv];
...
- (void)onTapChainsView:(id)tap {
    ...
    /// 国外选择链
    SQSEthereumChainsSelectPopView *v = [[SQSEthereumChainsSelectPopView alloc] init];
...
- (SQSSelectEtherChainsView *)chainsView {
    if (!_chainsView) {
        _chainsView = [[SQSSelectEtherChainsView alloc] init];

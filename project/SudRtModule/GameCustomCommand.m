...
- (void)onCallCustomCommand:(nonnull id<SudRt2GameCustomCommandHandle>)handle info:(nullable NSDictionary *)argv {
    // TODO: wdz 实现 demo 层需要实现的 custom command 功能
    for (id<SudRt2GameCustomCommandListener> customCommandListener in _customCommandListenerArray) {
        [customCommandListener onCallCustomCommand:handle info:argv];
    }
    [handle customCommandSuccess];
}

- (void)onCallCustomCommandSync:(nonnull id<SudRt2GameCustomCommandHandle>)handle info:(nullable NSDictionary *)argv {
    for (id<SudRt2GameCustomCommandListener> customCommandListener in _customCommandListenerArray) {
